...
FROM HIS.XKTJ_TJSFXX where 是否收费 = '已收费' AND 收费时间 > sysdate-2/60*24 order by 收费时间 asc, 处方号 asc";
        ...
        logInfo('检查申请挂号 post -> ', [$payData], 'charging999');
...
//            }
            ...
            foreach ($new_union_item as $key => $item){
                ...
                $sendChargingData = [
                    ...
                    'doctor' => $orderInfo['empl_code'] ?? '100132'
                ];
                $insertChargingData = [];
                foreach ($item as $v){
                    $insertChargingData[] =   [
